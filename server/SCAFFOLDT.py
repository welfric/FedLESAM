...
class SCAFFOLDT(Server):
    def __init__(
        ...
        print(
            " Var Reduction Param List  --->  {:d} * {:d}".format(
        ...
        if args.mode == "dg":
            ...
            self.local_iteration = self.args.local_epochs * (
                self.datasets.client_x[0].shape[0] / self.args.batchsize
            )
    ...
    def process_for_communication(self, client, Averaged_update):
        ...
        self.comm_vecs["Local_VR_correction"].copy_(
            self.c_params_list - self.c_i_params_list[client]
        )

    def global_update(self, selected_clients, Averaged_update, Averaged_model):
        # SCAFFOLD with TempNet (ServerOpt)
        # updated global c
