...
class Server(object):
    def __init__(
        ...
        print(
            "   Local Model Param List  --->  {:d} * {:d}".format(
        ...
        print(
            " Local Updated Param List  --->  {:d} * {:d}".format(
                self.clients_updated_params_list.shape[0],
                self.clients_updated_params_list.shape[1],
            )
        )
        ...
        print(
            "   Train/Test [loss, acc]  --->  {:d} * {:d}".format(
                self.train_perf.shape[0], self.train_perf.shape[1]
            )
        )
    ...
    def _see_the_divergence_(self, selected_clients, t):
        ...
        self.divergence[t] = torch.norm(
            self.clients_params_list[selected_clients] - self.server_model_params_list
        ) ** 2 / len(selected_clients)
    ...
    def _test_(self, t, selected_clients):
        ...
        if self.args.mode == "dg":
            ...
            testdataset = data.DataLoader(
                datas, batch_size=256, shuffle=False, num_workers=4
            )
            loss, acc = self._validate_(testdataset)
            self.train_perf[t] = [loss, acc]
            print(
                "  Targeted Test   ----    Loss: {:.4f},   Accuracy: {:.4f}".format(
                    self.train_perf[t][0], self.train_perf[t][1]
                ),
                flush=True,
            )
            # loss, acc = self._validate_(testdataset)
            ...
            traindataset = data.DataLoader(
                Dataset(
                    np.concatenate(self.datasets.client_x, axis=0),
                    np.concatenate(self.datasets.client_y, axis=0),
                    train=False,
            ...
            testdataset = data.DataLoader(
                Dataset(
                    self.datasets.test_x,
                    self.datasets.test_y,
                    train=False,
                    dataset_name=self.args.dataset,
                ),
                batch_size=1000,
            ...
            print(
                "    Test    ----    Loss: {:.4f},   Accuracy: {:.4f}".format(
                    self.test_perf[t][0], self.test_perf[t][1]
                ),
                flush=True,
            )
            # if acc>self.best:
            ...
            self._see_the_divergence_(selected_clients, t)
            print(
                "            ----    Divergence: {:.4f}".format(self.divergence[t]),
                flush=True,
            )
    ...
    def _summary_(self):
        ...
        print("##=============================================##")
        print(
            "     Communication round   --->   T = {:d}       ".format(
                self.args.comm_rounds
            )
        )
        print(
            "    Average Time / round   --->   {:.2f}s        ".format(
                np.mean(self.time)
            )
        )
        print(
            "     Top-1 Test Acc (T)    --->   {:.2f}% ({:d}) ".format(
                np.max(self.test_perf[:, 1]) * 100.0, np.argmax(self.test_perf[:, 1])
            )
        )
    ...
    def _save_results_(self):
        ...
        if not self.args.non_iid:
            root += "/{:s}-{:s}{:s}-{:d}".format(
                self.args.dataset, "IID", "", self.args.total_client
            )
        else:
            root += "/{:s}-{:s}{:s}-{:d}".format(
                self.args.dataset,
                self.args.split_rule,
                str(self.args.split_coef),
        ...
        divergence_file = divergence_dir + "/divergence-{:s}{:s}.npy".format(
    ...
    def train(self):
        ...
        for t in range(self.args.comm_rounds):
            ...
            selected_clients = self._activate_clients_(t)
            print(
                "============= Communication Round", t + 1, "=============", flush=True
            )
            print(
                "Selected Clients: %s"
                % (", ".join(["%2d" % item for item in selected_clients]))
            )
            # if self.args.mode=="dg":
            ...
            for client in selected_clients:
                if self.args.mode == "dg":
                    ...
                    dataset = data.DataLoader(
                        Dataset(
                            self.datasets.client_x[client],
                            self.datasets.client_y[client],
                            train=True,
                            dataset_name=self.args.dataset,
                        ),
                        batch_size=self.args.batchsize,
                ...
                self.clients_params_list[client] = self.received_vecs[
                    "local_model_param_list"
                ]
                self.postprocess(client)
            ...
            # calculate averaged model
            Averaged_update = torch.mean(
                self.clients_updated_params_list[selected_clients], dim=0
            )
            Averaged_model = torch.mean(
                self.clients_params_list[selected_clients], dim=0
            )

            self.server_model_params_list = self.global_update(
                selected_clients, Averaged_update, Averaged_model
            )
            set_client_from_params(
                self.device, self.server_model, self.server_model_params_list
            )
